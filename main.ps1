...
$r = ($pulled | Where-Object { $_.podcast.title -imatch "<NAME>"})
    > $r.episodes | select -First 10 | Select-Object -Property pubDate, title
...
$i = #
$pulled[$i].podcast
$e = .\test\invoke_brave_download.ps1 $pulled[$i].podcast $pulled[$i].episodes[0]
VLC play and exit with rate 1.5
& $configuration[1].vlc "$e" --rate 1.5 --play-and-exit

Showing the 3 latest episodes from each podcast:
$pulled | ForEach-Object { $_.episodes | select -First 3 | Select-Object -Property pubDate, title }
