...
param (
    ...
    [ValidateScript({ ![string]::IsNullOrEmpty($_.pubDate) -and [uri]::IsWellFormedUriString($_.enclosure.url, 'Absolute') })]
    [hashtable] $Episode
)
begin {
    ...
    function update_file_tags {
        ...
        begin {
            $published = get-date -date $Episode.pubDate
            $year = $published.Year
        ...
        process {
            ...
            if ([string]::IsNullOrEmpty($tags.Tag.Description)) {
                if (![string]::IsNullOrEmpty($Episode.description)) {
                    $tags.Tag.Comment = [System.Web.HttpUtility]::HtmlDecode($Episode.description)
                }
                else {
                    $tags.Tag.Comment = [System.Web.HttpUtility]::HtmlDecode($Episode.encoded)
                }
            }
...
process {
    ...
    #>
        Start-Process -FilePath $brave -ArgumentList "--autoplay-policy=user-gesture-required --new-tab $($Episode.enclosure.url)";
        ...
        # on the fence about updating the url since if anything fails then it is broke until repull
        # if ($Episode.enclosure.url -ne $urlformatted) {
        #     $Episode.enclosure.uri = $urlformatted
        # }
        # could add it and see if it exists and then use it and use old for backup 
        $Episode.Add('browserurl', $urlformatted)
    }
    ...
    if (![string]::IsNullOrEmpty($Episode.browserurl)) {
        ...
        Invoke-WebRequest -Uri $Episode.enclosure.uri -OutFile $download
    }
